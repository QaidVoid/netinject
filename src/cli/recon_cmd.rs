...
pub async fn run(cli: &Cli) -> Result<()> {
    ...
    let (session_id, session) =
        helpers::create_session(&store, target, "recon", &serde_json::to_string(&cfg)?)?;
    ...
    let adapter = registry
        .get("httpx")
        .ok_or_else(|| anyhow::anyhow!("httpx adapter not registered"))?;

    if !adapter.check_available()? {
        anyhow::bail!(
            "httpx is not installed. Install it: go install -v github.com/projectdiscovery/httpx/cmd/httpx@latest"
        );
    }

    if cli.dry_run {
        println!("Would run: httpx -u {target} -json -silent");
